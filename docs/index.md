...
- [2. カスタムクエリの作成](#2-カスタムクエリの作成)
- [3. 設定ファイルへの記載](#3-設定ファイルへの記載)
- [4. 起動と確認](#4-起動と確認)
- [うまくいかないとき](#うまくいかないとき)
...
redntfy が追跡するチケットの範囲は、Redmine のカスタムクエリで決まります。
...
この番号を次の手順で設定ファイルに書きます。

![クエリ番号の確認](images/query-id.png)

## 3. 設定ファイルへの記載

`redntfy.exe` と同じフォルダに `redntfy.local.toml` を作成し、控えた情報を記載します。
...
- `api_key`：手順 1 で取得した API アクセスキー
- `query_ids`：手順 2 で控えたクエリ番号
...
タスクトレイのアイコンを左クリックして、チケット一覧が表示されれば設定は完了です。
以降はカスタムクエリに一致するチケットが新着・更新されるたびに通知が届きます。

## うまくいかないとき

- 一覧に 1 件も表示されない  
  カスタムクエリの条件に一致するチケットが本当にあるか、Redmine の画面で同じクエリを開いて確認してください。
