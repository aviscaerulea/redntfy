...
- 一覧は `WS_EX_NOACTIVATE` の自前ポップアップ（クラス `redntfy_list`）で、フォーカスを一切奪わない  
  ...
  開く：ホバー（`WM_MOUSEMOVE`＋ワンショット `IDT_HOVER_TRIGGER` のデバウンス、`hover_delay_ms`。
  トレイメニュー「マウスホバーで一覧を自動表示」が OFF なら無効。レジストリ永続化。既定 ON）
  または左クリック（即時。トグル OFF でも開ける）。閉じる：アイコンとポップアップ両方からの離脱
  （`IDT_LIST_WATCH` の 200ms ポーリング 2 tick）・表示中の左クリック（トグル）・行クリック。
  起点によらず同一ルールで、状態は「表示中か否か」と「ホバー起点か」（`g_hoverShownAt`）のみ。
  ...
  明示クローズ後はカーソルがアイコンを離れるまでホバー再アームを保留する。（乗ったままの
  微動で開き直るのを防ぐ。左クリックでの表示は保留の対象外。監視は `IDT_LIST_WATCH` が兼ねる）
