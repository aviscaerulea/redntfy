...
- ホバー一覧は `WM_MOUSEMOVE`＋ワンショット `SetTimer`（`IDT_HOVER_TRIGGER`）のデバウンス方式  
  ...
  ホバー ON 中はトレイの左クリックを完全に無視し、一覧の表示経路をホバーに一本化する。
  （保留中のホバートリガーも殺さない。OFF なら従来どおり左クリックで表示）
  ホバー一覧の表示中は WH_MSGFILTER（MSGF_MENU）フックでアイコン上の左ボタンを不発にする。
  （TrackPopupMenu はメニュー外の押下で閉じるため、フックなしではクリックが閉じる操作として効く）
  明示操作で閉じた後は、カーソルがアイコンから一度離れるまでホバー再表示を抑止する。
  （`IDT_HOVER_REARM` の 200ms ポーリングで離脱を検出。乗ったままの微動で開き直るのを防ぐ）
  ホバー OFF のクリック起点一覧は、閉じた直後 500ms の左クリックを抑止する。
  （アイコンクリックの解除はメニュー解除 → `WM_LBUTTONUP` の順で届くため）
  オーバーフロー領域ではホバー ON 中に一覧を開く手段がなくなる。（ホバー OFF への切替か
  アイコンのピン留めで回避。README の制限事項に記載済み）
