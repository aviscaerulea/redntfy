...
未処理チケット一覧の各行には、期日、担当、プロジェクト、期日までの残り日数をアイコン付きで表示します。未読のチケットと期日のあるチケットは太字で強調します。新着として通知したチケットには ✨ を付けます。未処理の件数はフッタに表示します。行をクリックすると、ブラウザでそのチケットを開いて既読にします。行の右クリックでは、ピン留め、非表示、通常の 3 状態を順に切り替えます。
