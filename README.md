...
- ホバーで表示した一覧はカーソルが離れると自動で閉じ、直前のウィンドウへフォーカスが戻る
- ホバー表示は右クリックメニューの「ホバーで一覧を表示」で OFF にでき、再起動後も維持される
- トレイアイコンの左クリックでも一覧を表示し、表示中の左クリックは一覧を閉じる
- ホバー表示が ON の間は件数ツールチップを表示しない
...
- アイコンがオーバーフロー領域（隠れているインジケーター）にあるとホバー表示が機能しない（左クリックでは開ける）
