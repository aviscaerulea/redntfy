...
- トレイアイコンにカーソルを乗せる（デフォルト 0.1 秒）か左クリックで未処理チケット一覧を表示
- ホバー自動表示はメニューの「マウスホバーで一覧を自動表示」で OFF にできる（左クリックでは常に開ける）
- カーソルがアイコンと一覧の両方から離れると自動で閉じる（表示中の左クリックでも閉じる。ホバーで自動表示した直後 0.3 秒間を除く）
- 未処理の件数は一覧のフッタで、未読の有無はアイコンの赤バッジで確認できる
