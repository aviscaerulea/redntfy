...
- トレイアイコンにカーソルを乗せたままにしても同じ一覧を表示（デフォルト 0.25 秒。カーソルが離れると自動で閉じ、直前のウィンドウへフォーカスが戻る）
- ホバー表示の ON/OFF は右クリックメニューの「ホバーで一覧を表示」で切り替え（デフォルト ON、再起動後も維持。ON の間は件数ツールチップを表示しない）
...
| `[app]` | `hover_delay_ms` | ホバーで一覧を表示するまでの遅延（ミリ秒、0〜5000。デフォルト 250、0 で即時） |
...
- ホバーでの一覧表示は、トレイアイコンが「隠れているインジケーター」（オーバーフロー領域）にあると機能しない（アイコンをタスクバーへドラッグしてピン留めすると使える）
