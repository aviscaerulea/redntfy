...
システムトレイから Redmine チケットの通知と一覧表示を行う軽量常駐アプリです。
...
未処理チケット一覧の各行には、期日、担当、プロジェクト、最終更新からの経過時間をアイコン付きで表示します。未読のチケットと期日のあるチケットは太字で強調します。新着として通知したチケットには ✨ を付けます。未処理の件数はフッタに表示します。行をクリックすると、ブラウザでそのチケットを開いて既読にします。行の右クリックでは、ピン留め、非表示、通常の 3 状態を順に切り替えます。
...
アンインストールしても、以下の 3 つが残ります。不要であれば手動で削除してください。

- データフォルダ `%LOCALAPPDATA%\redntfy`（検知済みチケットの記録、ピン留め、非表示、ログ）
...
まずは設定なしで使い始めてください。
対象を絞り込みたくなったら、カスタムクエリを作成します。次に `query_ids` を設定します。
