...
- トレイアイコンにカーソルを乗せるだけでも一覧を表示（表示までの時間は調整可能、OFF にもできる）
...
- トレイアイコンにカーソルを乗せると未処理チケット一覧を表示（デフォルト 0.2 秒で表示）
- 一覧はカーソルが離れると自動で閉じ、直前のウィンドウへフォーカスが戻る
- ホバー表示は右クリックメニューの「ホバーで一覧を表示」で OFF にでき、再起動後も維持される
- ホバー表示が ON の間は件数ツールチップと左クリックが無効になり、OFF では左クリックで一覧を表示
- 行の左クリックでブラウザ表示して既読化、右クリックでピン留め → 非表示 → 通常を切り替え
- 非表示チケットはグレー表示になり、通知と未処理件数から除外（メニューの「非表示チケットを除外」で一覧からも消せる）
...
| `[app]` | `hover_delay_ms` | ホバーで一覧を表示するまでの遅延（ミリ秒、0〜5000。デフォルト 200、0 で即時） |
...
- アイコンがオーバーフロー領域（隠れているインジケーター）にあるとホバー表示が機能せず、一覧を開けない
- 回避策はアイコンをタスクバーへドラッグしてピン留めするか、ホバー表示を OFF にして左クリックで開く
